...
\subsection{Construct the graph views and reconciliation comparison}

For the native directed graph, namespace every local node by document and GSW
filename. Add entity, space/time, and verb-phrase nodes with their original
attributes. For every grounded QA answer, add an edge from the corresponding
verb-phrase node to the answer node and use the QA ID as the multiedge key.
Preserve the question text and originating document ID on the edge.

For the unreconciled projection, collect the distinct answer nodes reached
from each verb-phrase. Add an undirected edge for every pair in that local
neighborhood and increment its weight when the pair occurs under another
verb-phrase. Keep the occurrence UIDs unchanged. Reconciliation is a separate
mapping from occurrence UID to proposed global entity ID; relabel the
unreconciled projection with that mapping and aggregate parallel edges. Test
projection and aggregation on two hand-written documents before processing
the supplied corpus.

Compare three conditions: no cross-document reconciliation, the supplied
exact-surface baseline, and your conservative method. Inspect proposed merges
before interpreting the global topology. A large giant component is not
evidence that the reconciliation is correct.
...
Implement a loader for the graph views defined above. Preserve node type,
document provenance, role/state text, QA ID, and direction in the native
graph. Keep reconciliation as an explicit mapping so the same unreconciled
projection can be evaluated under different identity rules.
...
and conservative projection; the native-graph measurements are additional.

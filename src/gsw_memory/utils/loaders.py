...
def load_operator_outputs(log_dir: Union[str, Path]) -> List[Dict[str, Dict]]:
    ...
    try:
        ...
        print(
            f"✅ Loaded operator outputs: {len(all_documents_data)} documents, "
        ...
        raise LoaderError(
            f"Failed to load operator outputs from {log_dir}: {str(e)}"
        ) from e
...
def load_reconciled_gsw(log_dir: Union[str, Path]) -> GSWStructure:
    ...
    try:
        log_dir = Path(log_dir)
        reconciled_file = (
            log_dir / "reconciled_output" / "reconciled" / "global_reconciled.json"
        )
        ...
        print(
            f"✅ Loaded reconciled GSW: {len(reconciled_gsw.entity_nodes)} entities, "
        ...
        raise LoaderError(
            f"Failed to load reconciled GSW from {log_dir}: {str(e)}"
        ) from e
...
def load_entity_summaries(log_dir: Union[str, Path]) -> Dict[str, str]:
    ...
    try:
        ...
        raise LoaderError(
            f"Failed to load entity summaries from {log_dir}: {str(e)}"
        ) from e
...
def print_log_summary(log_dir: Union[str, Path]) -> None:
    ...
    stages = {
        ...
        "Reconciled GSW": log_dir
        / "reconciled_output"
        / "reconciled"
        / "global_reconciled.json",

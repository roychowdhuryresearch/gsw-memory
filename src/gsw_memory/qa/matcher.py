...
class EntityMatcher:
    ...
    def find_matching_entities(
        ...
        for entity in gsw.entity_nodes:
            ...
            if any(
                ...
                or clean_entity_name.lower() in name.lower()
                or name.lower() in entity.name.lower()
                or name.lower() in clean_entity_name.lower()
    ...
    def _find_connected_entities(
        self, entity: EntityNode, gsw: GSWStructure
    ) -> List[EntityNode]:
        """
                ...
                for question in vp.questions:
                    for answer_id in question.answers:
                        if (
                            ...
                            and answer_id != "None"
                            and not (
                                isinstance(answer_id, str)
                                and answer_id.startswith("TEXT:")
                            )
                        ):

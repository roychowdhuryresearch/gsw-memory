#!/usr/bin/env python3
"""
ABLATION STUDY: NO RERANKING AT ALL

This script evaluates multi-hop QA WITHOUT any reranking.
Both chain reranking and Q&A pair reranking are disabled.

ABLATION:
- ✅ Question decomposition
- ✅ Chain-following retrieval
- ❌ Chain reranking (voyage reranker disabled)
- ❌ Q&A reranking (voyage reranker disabled for Q&A pairs)
...
# ABLATION: Use modified version without ANY reranking
from multi_hop_qa_chains_no_rerank import ChainFollowingMultiHopQA
...
LOG_FILE = LOG_DIR / f"multihop_qa_NO_RERANKING_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
...
class ChainBatchedMultiHopQAEvaluator:
    ...
    def __init__(self, num_documents: int = 200, num_questions: int = 20, verbose: bool = False,
        ...
        # ABLATION: Disable BOTH chain and Q&A reranking
        self.qa_system = ChainFollowingMultiHopQA(
            ...
            use_bm25=use_bm25
            # Q&A reranking is disabled in the imported module
    ...
    def save_results(self, results: List[ChainBatchedEvaluationResult], overall_metrics: Dict[str, float], 
        ...
        """
        output_file = LOG_DIR / f"multihop_qa_NO_RERANKING_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        ...
        output_data = {
            "evaluation_info": {
                ...
                "ablation": "NO_RERANKING",
                "ablation_description": "NO reranking at all - both chain and Q&A reranking disabled"
            },
...
def main(verbose: bool = False):
    ...
    Args:
        verbose: Whether to show detailed output
    """
    console.print("\n[bold cyan]🚀 ABLATION: Multi-Hop QA WITHOUT Any Reranking[/bold cyan]")
    console.print("[yellow]ABLATION: NO reranking - both chain and Q&A reranking disabled[/yellow]")

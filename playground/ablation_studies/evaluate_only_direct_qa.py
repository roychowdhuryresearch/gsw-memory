#!/usr/bin/env python3
"""
ABLATION STUDY: ONLY DIRECT Q&A SEARCH

This script evaluates multi-hop QA using ONLY direct Q&A search.
Only direct Q&A search is used (no entity-based search).
...
- ✅ Chain reranking
- ✅ Q&A reranking
- ❌ Entity-based search (only direct Q&A search used)
...
# ABLATION: Use modified version with only direct Q&A search
from multi_hop_qa_chains_only_direct_qa import ChainFollowingMultiHopQA
...
LOG_FILE = LOG_DIR / f"multihop_qa_ONLY_DIRECT_QA_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
...
class ChainBatchedMultiHopQAEvaluator:
    ...
    def save_results(self, results: List[ChainBatchedEvaluationResult], overall_metrics: Dict[str, float], 
        ...
        """
        output_file = LOG_DIR / f"multihop_qa_ONLY_DIRECT_QA_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        ...
        output_data = {
            "evaluation_info": {
                ...
                "ablation": "ONLY_DIRECT_QA",
                "ablation_description": "Only direct Q&A search used (no entity-based search)"
            },
...
def main(verbose: bool = False):
    ...
    Args:
        verbose: Whether to show detailed output
    """
    console.print("\n[bold cyan]🚀 ABLATION: Multi-Hop QA with ONLY Direct Q&A Search[/bold cyan]")
    console.print("[yellow]ABLATION: Only direct Q&A search (no entity-based search)[/yellow]")

#!/usr/bin/env python3
"""
ABLATION STUDY: NO Q&A RERANKING

This script evaluates multi-hop QA WITHOUT Q&A pair reranking.
Q&A pairs are sorted by their original retrieval scores (entity_score or
similarity_score) instead of being reranked by voyage.
...
# ABLATION: Use modified version without Q&A reranking
from multi_hop_qa_chains_no_qa_rerank import ChainFollowingMultiHopQA
...
LOG_FILE = LOG_DIR / f"multihop_qa_NO_QA_RERANK_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
...
class ChainBatchedMultiHopQAEvaluator:
    ...
    def save_results(self, results: List[ChainBatchedEvaluationResult], overall_metrics: Dict[str, float], 
        ...
        """
        output_file = LOG_DIR / f"multihop_qa_NO_QA_RERANK_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        ...
        output_data = {
            "evaluation_info": {
                ...
                "ablation": "NO_QA_RERANKING",
                "ablation_description": "Q&A pairs are NOT reranked; sorted by original retrieval scores"
            },
...
def main(verbose: bool = False):
    ...
    Args:
        verbose: Whether to show detailed output
    """
    console.print("\n[bold cyan]🚀 ABLATION: Multi-Hop QA WITHOUT Q&A Reranking[/bold cyan]")
    console.print("[yellow]ABLATION: Q&A pairs not reranked; sorted by original retrieval scores[/yellow]")
